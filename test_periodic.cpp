...
int main (int argc, char **argv) try
{
    ...
    Vec3_t pos(nx*0.5,0.0,0.0);
    Vec3_t pos1(nx*0.5,3.0*R,0.0);
    Vec3_t dxp(0.0,2.0*R,0.0);
    ...
    while(dom.Time<Tf)
    {
        ...
        for(size_t i=0;i<dom.Particles.size();i++)
        {
            ...
            dom.Particles[i].Leave(modexy,Box);
        }
        ...
        #pragma omp parallel for schedule(static) num_threads(Nproc)
        for(size_t i=0; i<dom.GhostParticles.size();++i)
        {
            DEM::Disk *Pa = &dom.GhostParticles[i];
        ...
        for(size_t i=0; i<dom.ListofContacts.size();++i)
        {
            ...
            if(!dom.Particles[ip1].IsFree() && !dom.Particles[ip2].IsFree()) continue;
